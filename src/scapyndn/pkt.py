...
from scapy.layers.x509 import ECDSASignature, X509_SubjectPublicKeyInfo
...
class NdnStrLenField(StrLenField):
    ...
    def i2m(self, pkt, x):
        # Support for int and float if someone uses
        # value=255 or value=100.2 in NameComponent
...
class NdnTypeField(Field):
    ...
    def __init__(self, default, valid_types=None, fmt="!H"):  # noqa: E501
        # If valid_types=None then all types expected, ex in NameComponent
    ...
    # TODO: Show any known type name in bracket for that
    # particular packet (something like enum)
    def i2repr(self, pkt, x):
        if x is None:
            return ""
        return x
    ...
    def getfield(self, pkt, s):
        ...
        try:
            if x < 253:
                rem, val = s[1:], self.m2i(pkt, struct.unpack(">B", s[:1])[0])
            elif x == 253:
                rem, val = s[3:], self.m2i(pkt, struct.unpack(">H", s[1:3])[0])
            elif x == 254:
                rem, val = s[5:], self.m2i(pkt, struct.unpack(">L", s[1:5])[0])
            else:
                rem, val = s[9:], self.m2i(pkt, struct.unpack(">Q", s[1:9])[0])
        except Exception:
            return s, None
...
class _NdnPacketList_metaclass(Packet_metaclass):
    ...
    def __new__(cls, name, bases, dct):
        ...
        if "TypeToCls" in dct:
            ...
            fields_desc.append(
                PacketListField("value", [], dct["PktCls"],
                                length_from=lambda pkt: pkt.length)
            )
        else:
            fields_desc.append(
        ...
        return super(_NdnPacketList_metaclass, cls). \
...
class _NdnNonNegativeInteger_metaclass(Packet_metaclass):
    ...
    def __new__(cls, name, bases, dct):
        ...
        fields_desc.append(NdnLenField())
        fields_desc.append(NonNegativeIntField("value", 0,
                                               length_from=lambda pkt:
                                               pkt.length))

        dct["fields_desc"] = fields_desc

        return super(_NdnNonNegativeInteger_metaclass, cls). \
            __new__(cls, name, bases, dct)


class NonNegativeIntBase(NdnBasePacket,
                         metaclass=_NdnNonNegativeInteger_metaclass):
...
def bind_cls_to_name(name_uri, num_after_name, cls):
    ...
    such as ControlParameters.
        ...
        * num_after_name: int such as 0 i.e. immediately after the name_uri
        * cls: cls such as IBF
    """
...
def bind_component_cls_dict_to_name(name_uri, num_after_name, types_to_cls):
    ...
    """
    When dissecting Name, it could have Packets other than NameComponent*
    such as ControlParameters
    ...
    """
    class GuessPktListNameComponent(NameComponent,
...
def bind_component_cls_to_name(name_uri, num_after_name, pkt_cls):
    ...
    """
    class PktListNameComponent(NameComponent,
                               metaclass=_NdnPacketList_metaclass):
        NdnType = TYPES["GenericNameComponent"]
        PktCls = pkt_cls
...
class Name(NdnBasePacket):
    ...
    TYPES_TO_CLS = {
        ...
        TYPES["ParametersSha256DigestComponent"]:
    ...
    # default is overridden to NameComponent in fields_desc,
    # but keep as Block here to match parent
    def guess_ndn_packets(self, lst, cur, remain, types_to_cls, default=Block):
        """
        ...
        if bound_name is not None:
            num_to_prefix = NAME_URI_TO_NAME_COMPONENT_VALUE_CLS[bound_name]
            if comp_num_after_prefix in num_to_prefix:
                return num_to_prefix[comp_num_after_prefix]
...
class SignatureType(NdnBasePacket):
    ...
    SIG_TYPE_VALUES = {
        0: "DigestSha256", 1: "SignatureSha256WithRsa",
    ...
    fields_desc = [
        ...
        NdnLenField(),
        NonNegativeIntField("value", "",
                            length_from=lambda pkt: pkt.length,
                            enum=SIG_TYPE_VALUES)
    ]
...
# @TODO: Why use PktCls is here as value should not be a PacketList?
# Becasue ECSDASignature is a Packet we should use PacketField
# class ECDSASignatureValue(NdnBasePacket):
...
class DigestSha256SignatureValue(NdnBasePacket,
                                 metaclass=_NdnPacketList_metaclass):
    NdnType = TYPES["SignatureValue"]
    PktCls = DigestSha256
...
def bind_content_cls_to_data_name(name_uri, content_val_cls):
    ...
    """
    Creates a mapping NameURI->Content class w/ value as content_val_cls
    This Content class is then used during
    Data packet dissection when NameURI matches.
...
class Data(NdnBasePacket):
        ...
        if blk.type == TYPES["Content"]:
            ...
            for name in lst + [cur]:
                ...
                for n in NAME_URI_TO_CONTENT_CLS:
                    is_prefix, n_num_comp = name.is_prefix_of(n)
                    # print(n, name.to_uri(), is_prefix, n_num_comp)
    ...
    def guess_payload_class(self, payload):
        # In case we have two NDN packets back-to-back
        # What happens if the second packet is not Ndn?
        # App should bind that pattern if second packet not Ndn
        # Need same guess_payload_class for Interest and LpPacket
        return NdnGuessPacket
        # return conf.padding_layer
...
class LinkObject(Data):
    ...
    fields_desc = [
        NdnTypeField(TYPES["Data"]),
        NdnLenField(),
        # Do we use guess packets from Data here now,
        # or since ContentType is known that is not needed
...
class Nack(NdnBasePacket):
    ...
    fields_desc = [
        ...
        NdnLenField(),
        PacketListField("value", [], NackReason,
                        length_from=lambda pkt: pkt.length)
    ]
...
class LpPacket(NdnBasePacket):
    ...
    fields_desc = [
        ...
        PacketListField("value", [],
                        next_cls_cb=lambda pkt, lst, cur, remain:
                        pkt.guess_ndn_packets(lst, cur, remain,
                                              LpPacket.TYPES_TO_CLS),
                        length_from=lambda pkt: pkt.length)
    ]


class NdnGuessPacket(Packet):
    "Dummy packet for guessing NDN packets via bind_layers etc."
    ...
    def dispatch_hook(cls, _pkt=None, *args, **kargs):
        ...
        # Handle case when NdnGuessPacket called on already dissected packet
        # print("dispatch hook", _pkt, type(_pkt))
        if _pkt is not None:
            blk = TypeBlock(_pkt)
            if blk.type in NdnGuessPacket.TYPES_TO_CLS:
                return NdnGuessPacket.TYPES_TO_CLS[blk.type]
        # Maybe return Ether here instead of Block
        # if length does not match value
        return Block
